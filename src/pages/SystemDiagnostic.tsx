...
export default function SystemDiagnostic() {
  ...
  const sections: { key: string; title: string; icon: typeof Activity }[] = [
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={copyDiagnostic} disabled={!result} className="gap-2">
            <Copy className="w-4 h-4" /> Copy
          </Button>
          <Button variant="outline" onClick={exportDiagnostic} disabled={!result} className="gap-2">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Completed in {result.total_time_ms}ms · {result.errors.length} errors · {result.warnings.length} warnings
                  </p>
                </div>
              </div>
              <Button variant="ghost" size="icon" onClick={runDiagnostic} disabled={loading}>
          ...
          {sections.map((s) => {
            ...
            return (
              <ServiceSection
                ...
                details={data}
                checkedAt={data.checked_at as string | undefined}
                responseTime={data.response_time_ms as number | undefined}
              />
            );
          })}
        </div>
