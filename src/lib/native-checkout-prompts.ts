...
- **Dispatch gate ligado ao process-events (Passo T)**: \`process-events\` chama \`decideDispatch()\` antes de enviar para Meta/Google/TikTok/GA4. Itens bloqueados viram \`status=skipped\` em \`event_queue\` e geram registro em \`event_deliveries\` com \`request_json.dispatch_decision='blocked'\` + \`reasons[]\` — sem PII, sem credenciais. \`test_mode\`/\`dry_run\` no body do worker força gate em modo seguro. Painel "Testar dispatch" em \`/destination-registry\` chama a Edge Function \`dispatch-test\` (verify_jwt) que NUNCA dispara para provedor real.
- **Hardening operacional Passo U**: cache TTL curto (\`REGISTRY_CACHE_TTL_MS\`, default 60s) no \`process-events\` para o registry, evitando depender da vida do isolate. Workspace selector explícito em \`/destination-registry\` e \`/data-reuse-center\` quando o usuário tem múltiplas workspaces (single-workspace mantém UX atual). Auditoria dedicada em \`dispatch_decision_log\` com RLS member-read e RPC \`record_dispatch_decision\` (service-only) + RPC \`list_dispatch_decisions\` (filtros provider/destination/decision, sem PII/segredos). \`test_mode\`/\`dry_run\` é hard-stop antes de qualquer \`fetch\` — Meta/Google/TikTok/GA4 NUNCA recebem chamada real. Reasons detalhadas (allow/block/dry_run/test_mode/fallback) ficam visíveis no painel de auditoria com filtros.
