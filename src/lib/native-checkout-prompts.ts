...
## 🔐 Controles de release endurecidos (Passo M+N — sincronizado)
...
- **Fast-path por gateway (Passo N)**: WooCommerce/Braip/CartPanda/PerfectPay seguem \`gateway-fast-path-guides.ts\` — webhook canônico \`?provider=generic\`, secret HMAC obrigatório, propagação de \`root_order_code\`/\`step_key\`/\`external_reference\`.
- **PII report + audit viewer**: confira em \`/pii-release-report\` e \`/audit-logs\` (com redaction client-side de email/CPF/CNPJ/JWT/IP).
- **Relatório operacional**: status consolidado em \`/release-report\` (inclui marcador \`RLS semantic audit\` quando indisponível por falta de PGHOST em CI).
`;
